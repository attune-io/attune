...
| **Version-aware memory limit decrease (clamp vs apply)** | Covered in **Go E2E** `TestE2E_MemoryLimitDecrease_VersionAware` (nightly 1.32–1.37 matrix): Guaranteed pod with oversized limit, `RequestsAndLimits` + `allowDecrease`; asserts limit drops on 1.35+ and stays clamped on 1.33–1.34. Unit tests still own clamp math and version parsing. | `test/e2e-go/e2e_test.go`, `internal/resize/engine_test.go` |
...
| **Datadog / CloudWatch query shape** | Query builders, site allowlist, and SEARCH sanitization stay unit-tested. Go E2E proves an exclusive Datadog or CloudWatch policy does **not** fall back to cluster Prometheus: Ready=`MetricsUnavailable` and `withRecommendations=0` (fake API key / no AWS creds). | `internal/metrics/datadog_test.go`, `internal/metrics/cloudwatch_test.go`, `TestE2E_DatadogSource_DoesNotUseClusterPrometheus`, `TestE2E_CloudWatchSource_DoesNotUseClusterPrometheus` |
...
classifier (`scripts/test_release_image_tags.sh`), the cert-manager
manifest download classifier
(`scripts/test_e2e_download_cert_manager.sh`), and the Dependabot
rebase-outdated classifier
(`scripts/test_rebase_outdated_dependabot.sh`). The last one locks
App-token git rebase (no `@dependabot rebase` comments). Those catch a `v`-prefix
