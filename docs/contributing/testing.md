...
| **Datadog / CloudWatch query shape** | Query builders, site allowlist, and SEARCH sanitization stay unit-tested. Go E2E proves an exclusive Datadog or CloudWatch policy does **not** fall back to cluster Prometheus: Ready=`PrometheusUnavailable` and `withRecommendations=0` (fake API key / no AWS creds). | `internal/metrics/datadog_test.go`, `internal/metrics/cloudwatch_test.go`, `TestE2E_DatadogSource_DoesNotUseClusterPrometheus`, `TestE2E_CloudWatchSource_DoesNotUseClusterPrometheus` |
