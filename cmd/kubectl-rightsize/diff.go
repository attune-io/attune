...
// printDiff lists all RightSizePolicies and shows resource change diffs for
...
func printDiffItems(items []unstructured.Unstructured, output string) {
	...
	if !hasOutput {
		fmt.Println("No recommendations available. Run 'kubectl rightsize status' to check policy status.")
	}
}
