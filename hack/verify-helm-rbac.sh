...
HELM_CHART="$REPO_ROOT/charts/attune"
...
echo "+++ helm (charts/attune/templates/clusterrole.yaml)" >&2
diff <(echo "$kustomize_rules") <(echo "$helm_rules") >&2 || true
echo "" >&2
echo "Fix: update charts/attune/templates/clusterrole.yaml to match" >&2
