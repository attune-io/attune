...
| [05-cluster-defaults.yaml](05-cluster-defaults.yaml) | Cluster Prometheus address, no token | Recommend |
| [06-multi-workload-selector.yaml](06-multi-workload-selector.yaml) | Label selector targeting many Deployments | Recommend |
| [07-sidecar-exclusion.yaml](07-sidecar-exclusion.yaml) | Skip service mesh sidecars with `excludedContainers` | Recommend |
...
| [13-multi-datasource.yaml](13-multi-datasource.yaml) | Mimir, Thanos, per-policy bearer token, and TLS | Recommend |
...
|-----------|---------------|
| [multi-namespace/](multi-namespace/) | Dev/staging/prod policies with Kustomize overlays, different aggressiveness per environment. Shared Prometheus address, no token |
| [full-stack/](full-stack/) | Everything needed for one app: namespace, Deployment, ServiceMonitor, AttuneDefaults, AttunePolicy, and Grafana dashboard ConfigMap. Prometheus address, no token |
| [prometheus-auth/](prometheus-auth/) | Per-namespace token, one operator identity, and a namespace exception |
...
- Prometheus reachable inside the cluster. The manifests here use an
  address and no token. Authenticated Prometheus is
