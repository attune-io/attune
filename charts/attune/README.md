...
| fleetReport | object | `{"clusterId":"","configMapName":"attune-fleet-report","enabled":false,"interval":"5m","namespace":""}` | Optional per-cluster fleet summary ConfigMap for multi-cluster collectors. Writes a versioned JSON report (schemaVersion v1) for scripts like scripts/collect-fleet-reports.sh. |
...
| metrics.prometheusRule.fleetRecordingRules | object | `{"enabled":false}` | Recording rules for multi-cluster / federated Prometheus rollups. Safe to enable on single-cluster installs (cluster label may be empty). |
