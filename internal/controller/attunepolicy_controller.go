...
const (
	...
	annotationOriginalCPUPrefix          = "attune.io/original-cpu-request."
	annotationOriginalMemoryPrefix       = "attune.io/original-memory-request."
	annotationOriginalCPULimitPrefix     = "attune.io/original-cpu-limit."
	...
	annotationPolicy = "attune.io/policy"

	// finalizerName is the finalizer added to AttunePolicy resources to
...
func (r *AttunePolicyReconciler) now() time.Time {
...
func (r *AttunePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Step 1: Fetch the AttunePolicy CR.
	...
	if err != nil {
		...
		r.setFailedCondition(ctx, &policy, attunev1alpha1.ReasonInvalidConfig,
	...
	if len(workloads) == 0 {
		...
		r.setFailedCondition(ctx, &policy, attunev1alpha1.ReasonNoWorkloadsFound, "No matching workloads found; check that targetRef name or selector matches an existing workload in this namespace")
	...
	// Opt-in GitOps PR automation (Phase B). Uses template drift vs recommendations.
	// Safe to call with empty recs (sets NoDrift / Disabled). Skip Observe-only silence.
	if policy.Spec.UpdateStrategy.Type != attunev1alpha1.UpdateTypeObserve {
...
func (r *AttunePolicyReconciler) processWorkloads(
	ctx context.Context,
	...
	if isVPASource {
		...
		if vpaErr != nil {
			...
			if len(result.workloadErrors) < 10 {
				result.workloadErrors = append(result.workloadErrors, attunev1alpha1.WorkloadError{
					Workload: "*",
	...
	for _, workload := range workloads {
		g.Go(func() error {
			...
			var rec *attunev1alpha1.WorkloadRecommendation
			var qErrors int
...
func (r *AttunePolicyReconciler) setReadyCondition(
	policy *attunev1alpha1.AttunePolicy,
	workloadCount int,
	workloadsWithRecs int32,
	totalQueryErrors int,
	...
	minimumDP := r.getMinimumDataPoints(policy)
	reason := attunev1alpha1.ReasonInsufficientData
...
func (r *AttunePolicyReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&attunev1alpha1.AttunePolicy{}, builder.WithPredicates(specOrDeletePredicate{})).
